...
doc_train, y_train = read_data(data_path + "/task2-train-clean.txt")
print(time.process_time() - pt)

ids = np.arange(len(doc_train))
np.random.shuffle(ids)

# shuffle
X = [doc_train[i] for i in ids]
y = [y_train[i] for i in ids]
...
for k in range(1, nb_epoch+1):
    ...
    for n_batch in range(0,n_samples,batch_size):
        ...
        try:
            ...
            sys.exit(1)
        X = X.reshape(len(X), 1, maxfeatures, maxlen)#Comment for LSTM
        Y = np_utils.to_categorical(np.array(Y), n_classes)
        loss, acc = model.train_on_batch(X, Y)
        #print('x_train shape:', X.shape)
        epoch_accuracy += acc
        
        if n_batch%100 == 0: print("Accuracy ",acc, " in ",n_batch)
